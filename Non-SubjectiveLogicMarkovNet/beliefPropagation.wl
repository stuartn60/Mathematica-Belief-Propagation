...
(* ::Package::*)(* ::Input::Initialization::*)BeginPackage["beliefPropagation`" ]; 
...
normalizeClique[clique_, isMax_] :=Module[{newClique, sum},
        newClique = clique;
        sum = Total @ clique[[-1]];
        If[isMax == False && sum > 0,
            newClique[[-1]] = clique[[-1]] / sum ];
        newClique ]

indexToAssignment[index_, card_] :=
    (Reverse /@ Tuples @ Reverse @ (Range /@ card))[[index]]

assignmentToIndex[assignment_, card_] :=
Flatten[Position[(Reverse /@ Tuples @ Reverse @ (Range /@ card)),#]& /@  assignment]

factorProductOrSum[f1_, f2_, isMax_:False] :=
    Module[{vars, cards, varSubst, fcpd, ind2, ind2indx, \[ScriptCapitalD], pos, res0,op },
   vars = Union @ Flatten[First /@ {f1, f2}];
        cards = Flatten[Union /@ Flatten /@ Transpose @ Table[Extract[
            i[[2]], #]& /@ Position[First @ i, #]& /@ vars, {i, {f1, f2}}]];
        If[Length @ cards != Length @ vars,
            Print["Error in cardinalities ... Variables: ", vars, " ... Cardinalities: ",
                 cards];
            Abort[] ];
        (*varSubst = Thread[vars -> (ToExpression @ Alphabet[])[[Range @ Length @ vars]]];
    ind2 = Thread[{(ToExpression @ Alphabet[])[[Range @ Length @ cards]], cards}];*)
varSubst = Thread[vars ->  Array[c,Length@vars]];
ind2 = Thread[{ Array[c,Length@vars], cards}];
fcpd[f_, varvals_] := First @ Extract[Last @ f, Position[indexToAssignment[
Range @ (Times @@ f[[2]]), f[[2]]], varvals]];
ind2indx = Riffle[#, 1, {2, -1, 2}]& /@ ind2;
op=If[isMax==True,Plus,Times];
 \[ScriptCapitalD] =Flatten @ Table[op @@ {fcpd[f1, (First @ f1) /. varSubst],
fcpd[f2,(First @ f2) /. varSubst]},
 ## ]& @@ ind2indx;
pos = Position[Partition[Flatten @ Table[vars /. varSubst, ##]& @@
ind2indx, Length @ vars], #]& /@ indexToAssignment[Range @
    (Times @@ cards), cards];
res0 = Flatten @ Extract[\[ScriptCapitalD], pos];
{vars, cards, res0}]

factorMarginalization[a_, v_, isMax_] :=
    Module[{bVar, mapB, bCard, bVal, assignments, indxB},
        If[Or[First @ a == {}, v == {}],Return[a]];
        bVar = Complement[First @ a, v];
        mapB = Flatten[Position[First @ a, #]& /@ bVar];
        If[bVar == {},Print["Error:Resultant factor has empty scope"];
            Abort[]];
        bCard = a[[2, mapB]];
        assignments = indexToAssignment[Range @ Length @ Last @ a,a[[2]]];
        indxB = assignmentToIndex[assignments[[All, mapB]], bCard];
        bVal = If[isMax == True,
                Max @@ {Last /@ #}& /@ GatherBy[Thread[{indxB, Last @
                     a}], First],
                Total @@ {Last /@ #}& /@ GatherBy[Thread[{indxB, Last
                     @ a}], First]];
        {bVar, bCard, bVal}]

observeEvidence[f_, e_] :=
    Module[{vars, cards,pos1, pos2,e1,f1},
If[e=={},Return@f,f1=f];
vars = First /@ e;
...
(*{First@#,#[[2]],Flatten[If[Total@#==0,#,#/Total@#]&/@Partition[Last@#,First@#[[2]]]]}&/@f1*)
f1]
...
computeMarginal[v_, f_, e_] :=
    Module[{factorsEvidence, joint0, joint, marginal},
        If[Length @ f == 0,
...
If[Length @ Select[MemberQ[cnodes[[j]], #]& /@ cnodes[[i]],
# == True&] == Length @ cnodes[[i]],
Do[
...
createCliqueTree[f_, e_, method_:"Kruskal", print_:False] :=
    Module[{nodeEdges, nodeGraph, cliques0, cliques, cliqueNodes, cliqueEdges,
...
If[print,
Print[Graph[Join[Union @@ First /@ f, cliqueNodes], Flatten
@ MapThread[Thread[#1 \[UndirectedEdge] #2]&, {cliqueNodes, cliques}], VertexLabels-> "Name"]];Print[HighlightGraph[cliqueGraph, maxSpanningTree, GraphHighlightStyle-> "Thick "]]];
computeInitialPotentials[{cliques,pruneTree[{cliques, Normal @ AdjacencyMatrix @ maxSpanningTree}],
If[e == {},f ,observeEvidence[f, e]]}]]; 

printFactorAndCliqueGraphs[f_, e_, method_:"Kruskal"] :=
    Module[{},createCliqueTree[f, e, "Kruskal", True];{}]; 
...
cliqueTreeCalibrate[p_, isMax_] := Module[{i, j, k, m, cliques, cliqueEdges, fp, requiredEvidence, messages,
    ...
    While[ True,
            ...
            If[{i, j} == {0, 0},Break[]];
            edges = Complement[Flatten @ Position[cliqueEdges[[i]], 1
                ], {j}];
            fp = Fold[factorProductOrSum[#1, #2, isMax]&, cliques[[i]],
     messages[[edges, i]]];
            messages[[i, j]] = normalizeClique[factorMarginalization[
                fp, Complement[First @ fp, cliques[[i, 1]]], isMax], isMax]];
        Do[edges = Complement[Flatten @ Position[cliqueEdges[[k]],1], {k}];
            fp = Fold[factorProductOrSum[#1, #2, isMax]&, cliques[[k]],
     messages[[edges, k]]];
            cliques[[k]] = factorMarginalization[fp, Complement[First
                 @ fp, cliques[[k, 1]]], isMax],{k, Length @ messages}];
        {cliqueEdges, cliques}]

computeMarginalsBP[cliques_, isMax_] :=
    Module[{vars, pos1, pos2},
        vars = Union @ Flatten @ (First /@ cliques);
        pos1 = First /@ First /@ (Position[cliques[[All, 1]], #]& /@vars);
        pos2 = MapThread[Complement,{First /@ cliques[[pos1]], Thread[
            {Range @ Length @ vars}]}];
        MapThread[normalizeClique[factorMarginalization[#1, #2, isMax],
 isMax]&, {cliques[[pos1]], pos2}]]

(*Sum (isMax is False) produces Normalised probabilities,Max (isMax is True) produces Unnormalised Natural Logarithms*)

computeExactMarginalsBP[f_, e_:{}, isMax_:False, method_:"Kruskal",
 print_:False]:=
computeMarginalsBP[Last @ cliqueTreeCalibrate[createCliqueTree[f,
         e, method, print], isMax], isMax]; 

maxDecoded[marginals_] :=Module[{res},
        res = Fold[Join[#1, First @ Position[#2, Max @ #2]]&, {}, Last
             /@ marginals];
...
getNextClusters[p_, messages_, oldMessages_, m_, useSmart_] :=
    If[useSmart == False,
        naiveGetNextClusters[p, m],
        smartGetNextClusters[p, messages, oldMessages, m]]

smartGetNextClusters[p_, messages_, oldMessages_, m_] :=
    Module[{pos, m1, from, to},
        pos = Position[First @ p, 1];
        m1 = Mod[m + 1, Length @ pos] + 1;
        Reverse @ pos[[m1]]]

createClusterGraph[f_, e_] :=
    Module[{f1, clusterNodes, factorNodes, edges0, edges, ajm},
        f1 = observeEvidence[f, e];
        clusterNodes = Range @ Length @ f1;
        edges = Flatten @ MapThread[Thread[#1 \[UndirectedEdge] #2]&, Transpose @ Select[
            Transpose @ {clusterNodes, First /@ f1}, Length @ #[[2]] > 1&]];
      {Normal @ AdjacencyMatrix @ Graph[clusterNodes, edges, VertexLabels
             -> "Name"], f1} ]

checkConvergence[new_, old_, threshhold_:10^-6] :=
    If[Or @@ Thread[Flatten @ Abs[new - old] < threshhold],
        True,False]

clusterGraphCalibrate[p_, isMax_, useSmartMP_, threshhold_:10^-6] :=
    Module[{clusterEdges, clusters, vars, card, messageVars, messageCards,
         messageVals, messages, tic, iter, i, j, lastMessages, prevMessage, edges,
         fp, sum, pos, clusterList, m},
        clusterEdges = First @ p;
        clusters = Last @ p;
        pos = Position[clusterEdges, 1];
        vars = Union @ Flatten @ (First /@ clusters);
        card = First /@ (Extract[clusters[[All, 2]], #]& /@ Position[
            First /@ clusters, #]& /@ vars);
        messageVars =Table[
                If[clusterEdges[[i, j]] == 1,
                    Intersection[clusters[[i, 1]], clusters[[j, 1]]],
                    {}],
                {i, Length @ clusters},
                {j, Length @ clusters}];
        messageCards =Table[
                If[clusterEdges[[i, j]] == 1,card[[messageVars[[i, j]]]],{}],
                {i, Length @ clusters},
                {j, Length @ clusters}];
        messageVals = Table[
                If[clusterEdges[[i, j]] == 1,
                    Table[1, Times @@ messageCards[[i, j]]],
                    {0}],
                {i, Length @ clusters},
                {j, Length @ clusters}];
        messages = Table[{messageVars[[i, j]], messageCards[[i, j]], 
            messageVals[[i, j]]}, {i, Length @ clusters}, {j, Length @ clusters}];
        tic = AbsoluteTime[];
        iter = 0;
        ...
        While[ True,
            ...
            fp = Fold[factorProductOrSum[#1, #2, isMax]&, clusters[[i]],
    messages[[edges, i]]];
            messages[[i, j]] = normalizeClique[factorMarginalization[
                fp, Complement[First @ fp, Intersection[clusters[[i, 1]], clusters[[j,1]]]],
 isMax], isMax];
            If[useSmartMP == True,lastMessages[[i, j]] = prevMessage];
            If[Mod[iter, Length @ pos] == 0,
                Print["LBP Messages Passed: ", iter];
                If[checkConvergence[messages, lastMessages, threshhold] == True,
                    Break[]];
        ...
        Print["Total number of messages passed: ", iter, " in ", AbsoluteTime[]
  - tic, " seconds"];
        clusterList = clusters;
        Do[clusterList[[Last @ pos[[m]]]] = factorProductOrSum[clusterList[[
            Last @ pos[[m]]]], messages[[First @ pos[[m]], Last @ pos[[m]]]], isMax ],
 {m, Length @ pos}];
        {clusterList, messages}
    ]

computeApproxMarginalsBP[f_, e_:{}, isMax_:False, useSmartMP_:True, threshhold_
    :10^-6] := computeMarginalsBP[First @ clusterGraphCalibrate[createClusterGraph[
        f, e], isMax, useSmartMP, threshhold], isMax]

convertSamiamNetwork[file_] :=
    Module[
        ...
        val2 = Flatten @ #& /@ val1;
        subst1 = Partition[Flatten @ #, Last @ Dimensions @ #]& /@ (Array[
            List, #]& /@ Dimensions /@ val1);
        subst2 = Table[Sort @ Thread[{RotateLeft @ txt1[[i]], #}]& /@
             subst1[[i]], {i, Length @ subst1}];
        subst3 = Flatten @ MapThread[Thread[#1 -> #2]&, {subst2, val2}];
        out1 = indexToAssignment[Range[Times @@ #], #]&/@RotateRight/@ (Dimensions/@ val1);
        out2 = Table[Sort @ Thread[{txt1[[i]], #}]& /@ out1[[i]], {i,Length @ out1}] /. subst3;
        Transpose[{txt1, RotateRight /@ (Dimensions /@ val1), out2}]]; 

sortFactorVariables[factors_]:=Module[{inVars,inCards,inVals,outVars,outCards1,outCards2,inIndex,inSubst,outIndex,outVals},
...
inIndex=indexToAssignment[Range[Times @@ #], #]&/@inCards;
inSubst=Flatten[Thread/@MapThread[Table[Sort@Thread[{#1,i}],{i,#2}]->#3&,{inVars,inIndex,inVals}],1];
outIndex=indexToAssignment[Range[Times @@ #], #]&/@outCards2;
...
renameFactors[factors_, list_] := Module[{vars1, vars2, vars},
        ...
        SortBy[Join[Thread[{vars}] /. Join[Thread["v" <> ToString @ #
            & /@ (First /@ list) -> Last /@ list], Thread["v" <> ToString @ #& /@
             (Last /@ list) -> First /@ list]], factors[[All, 2 ;; -1]], 2], First
             @ First @ #&]]
